...
if ($enquiryId && !$technicianId) {
   // 🔹 Case 1: Get enquiry with follow-up history (no JSON_ARRAYAGG)
...
if ($result->num_rows > 0) {
    ...
    $sql = "
        SELECT 
            e.enquiry_id, e.client_name, e.contact_person_name, e.contact_no1,
            e.enquiry_date, e.enquiry_status_id, s.status_name,
        ...
        LEFT JOIN enquiry_status s ON e.enquiry_status_id = s.id
        WHERE etm.technician_employee_id = ? AND e.is_active = 1
    ";
